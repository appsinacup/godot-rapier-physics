...
func test_description() -> String:
	return """Checks that RapierPhysicsServer2D.get_version() reports the version the addon ships
	as.
	"""

func test_name() -> String:
	return "RapierPhysicsServer2D | testing [get_version]"

func addon_version() -> String:
	var config := ConfigFile.new()
	if config.load("res://addons/godot-rapier2d/plugin.info.cfg") != OK:
		return ""
...
func test_start() -> void:
	var checks = func(_p_target: PhysicsUnitTest2D, p_monitor: GenericManualMonitor):
		...
		var version := str(RapierPhysicsServer2D.get_version())
