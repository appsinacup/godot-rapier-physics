...
# Distance from the platform centre the body starts at, and the force pushing it towards the
# platform. The body has to cross the centre to count as having passed through, and it must stay
# well inside its own grid cell so it can never reach the platform one row away that shares its
# collision layer.
...
func test_start() -> void:
	...
	for y in range(12):
		for x in range(30):
			...
			var towards := axis if blocked else -axis
			...
			rigid.position = center + (-axis if blocked else axis) * START_OFFSET
			rigid.collision_layer = 0
			rigid.collision_mask = 0
			rigid.set_collision_mask_value(x + 1, true)
			add_child(rigid)
			rigid.add_constant_force(towards * PUSH_FORCE)

			bodies.append(rigid)
			centers.append(center)
			approach.append(towards)
