...
cargo clippy --fix --allow-dirty --features="single-dim3,serde-serialize,test,simd-stable,experimental-threads,register-docs,api-4-7,api-custom"
export GDRUST_GODOT_BIN="${GDRUST_GODOT_BIN:-/Applications/Godot.app/Contents/MacOS/Godot}"
if [ "${OSTYPE#darwin}" != "$OSTYPE" ]; then
    cargo build --features="single-dim3,serde-serialize,test,simd-stable,experimental-threads,register-docs,api-4-7,api-custom"
    ...
    cargo build --features="single-dim3,serde-serialize,test,simd-stable,experimental-threads,register-docs,api-4-7,api-custom" --target=x86_64-unknown-linux-gnu
