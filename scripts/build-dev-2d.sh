...
cargo clippy --fix --allow-dirty --features="single-dim2,serde-serialize,test,experimental-threads,register-docs,api-4-7"
if [ "${OSTYPE#darwin}" != "$OSTYPE" ]; then
    cargo build --features="single-dim2,serde-serialize,test,experimental-threads,register-docs,api-4-7" # --verbose
    ...
    cargo build --features="single-dim2,serde-serialize,test,experimental-threads,register-docs,api-4-7" --target=x86_64-unknown-linux-gnu
