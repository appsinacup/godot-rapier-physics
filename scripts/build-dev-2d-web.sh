...
cargo clippy --fix --allow-dirty --features="single-dim2,serde-serialize,test,simd-stable,experimental-threads,register-docs,api-4-7,api-custom"
if [ "${OSTYPE#darwin}" != "$OSTYPE" ]; then
    cargo build --features="experimental-wasm,single-dim2,serde-serialize,test,api-4-7,api-custom" --target=wasm32-unknown-emscripten -Z build-std=std --verbose
