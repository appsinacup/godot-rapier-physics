...
if [ "${OSTYPE#darwin}" != "$OSTYPE" ]; then
    cargo build --features="experimental-wasm,single-dim2,serde-serialize,test,api-custom" --no-default-features --target=wasm32-unknown-emscripten -Z build-std=std --verbose
