...
# Stress test: a chain of boxes hung from a fixed anchor by pin joints. Links can never
# stretch past their rest length; the chain must not explode and must settle hanging.
...
func test_name() -> String:
	return "Joint3D | Pin (hanging chain)"
...
func test_start() -> void:
	...
	var checks := func(_t, m: GenericManualMonitor):
		...
		if m.frame >= SETTLE_FRAME:
			...
			if state.max_link > LINK + 0.25:
				m.add_test_error("max link length was %.3f (rest %.1f)" % [state.max_link, LINK])
			m.add_test_result(state.max_link < LINK + 0.25)
