	...
	var test_lambda: Callable = func(_p_target, p_monitor: GenericManualMonitor):
		...
		if position_expected.is_zero_approx() or normal_expected.is_zero_approx():
			var c := Geometry3D.segment_intersects_sphere(
				...
				51)
			if c.size() < 2:
				p_monitor.failed("No expected intersection for block %d index %d start %s move %s" % [test_block, test_index, start, move])
				return
		...
		if position_error > POSITION_TOLERANCE:
			p_monitor.failed("Position error for block %d index %d: error %.5f expected %s actual %s" % [test_block, test_index, position_error, position_expected, position_actual])
			return
