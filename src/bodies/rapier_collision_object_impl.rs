...
use crate::{rapier_wrapper::prelude::PhysicsEngine, servers::rapier_physics_server_extra::{PhysicsShapes, PhysicsSpaces}, Transform};

use super::rapier_collision_object::{CollisionObjectShape, IRapierCollisionObject, RapierCollisionObject};

impl RapierCollisionObject {
    pub(super) fn recreate_shapes(collision_object: &mut dyn IRapierCollisionObject, physics_engine: &mut PhysicsEngine, physics_shapes: &mut PhysicsShapes) {
        for i in 0..collision_object.get_base().get_shape_count() as usize {
            ...
            collision_object.create_shape(collision_object.get_base().shapes[i], i, physics_engine, physics_shapes);
            ...
            collision_object.get_base()
                .update_shape_transform(&collision_object.get_base().shapes[i], physics_engine, physics_shapes);
        }
    }
    ...
    pub(super) fn add_shape(
        ...
        if let Some(shape) = physics_shapes.get_mut(&p_shape) {
            shape.get_mut_base().add_owner(collision_object.get_base().get_rid());
        }
    ...
    pub(super) fn remove_shape_idx(
        ...
        if let Some(shape) = physics_shapes.get_mut(&shape.shape) {
            shape.get_mut_base().remove_owner(collision_object.get_base().get_rid());
        }
    ...
    pub(super) fn set_shape(
        ...
        collision_object.get_mut_base().shapes[p_index].collider_handle = collision_object.get_base().destroy_shape(
