...
struct CollatedObjectState<'a> {
    node_path: String,
    self_state: Option<ObjectExportState<'a>>,
    shape_owner_states: Vec<(i32, Vec<ObjectExportState<'a>>)>,
}
...
impl StateManager {
            ...
            VariantType::STRING => {
                match load_state.try_to::<String>() {
                    Ok(as_string) => {
                        ...
                        if let Ok(as_serde_json) = serde_json::from_str::<serde_json::Value>(&as_string) {
                            self.load_serde_json(as_serde_json, in_space, space);
                        }
                        else {
                            // Error handling here?
                            let decoded_string = Marshalls::singleton().base64_to_utf8(&as_string);
                            if let Ok(as_serde_json) = serde_json::from_str::<serde_json::Value>(&decoded_string.to_string()) {
                                self.load_serde_json(as_serde_json, in_space, space);
                            }
                        }                     
                    },
    ...
    fn load_serde_json (
        &mut self,
        mut json_state: serde_json::Value,
        space_rid: Rid,
    ...
    {        
        ...
        let json_map = json_state
            .as_object_mut()
            .ok_or_else(|| serde_json::Error::custom("Expected JSON object"))?;

        // Move a bunch of objects out of the state data.
        let root_nodepath = json_map
            .remove("root_node")
            .and_then(|v| v.as_str().map(|s| s.to_owned()))
            .ok_or_else(|| serde_json::Error::custom("Missing or invalid root_node"))?;
        
        // Naturally, when we pull a value out of a map by key, the key doesn't get preserved in the result;
        // as such, once we pull the space state out, we need to re-wrap it to restore the outer key,
        // which is necessary for serde_json to properly deserialize the data into an object.
        let rapier_space_inner = json_map
            .remove("RapierSpace")
            .ok_or_else(|| serde_json::Error::custom("Missing RapierSpace"))?;
        let rapier_space = serde_json::json!({ "RapierSpace": rapier_space_inner});
        
        let physics_server_id = json_map
            .remove("physics_server_id")
            .ok_or_else(|| serde_json::Error::custom("Missing physics_server_id"))?;
       
        // First, we import the space state; that's because space-state import has functionality to compare the new, loaded
        // space narrowphase data with its previous data; this is necessary because it lets the space judge which areas
        // have stale contact events, which need to be resolved before we load the areas' new states.   
        let space_state: ObjectImportState = serde_json::from_value(rapier_space)
            .map_err(|err| {
                panic!("Failed to deserialize space state: {}", err);
            })?;
        ...
        let Some(root_node) = self.base().try_get_node_as::<Node>(&root_nodepath.clone()) else {
            return Err(serde_json::Error::custom("Couldn't find the root node specified in imported state!"))
        };

        let physics_nodes = StateManager::get_all_physics_nodes_in_space(&root_node, space_rid);

        if let Some(serde_json::Value::Object(objects_state)) = json_state.get("physics_objects_state") {
            // Store all our node keys here. When we load state for a node, we can delete that node from this set.
            let mut nodes_pending_load: HashSet<_> = objects_state.keys().cloned().collect();
            ...
            for nodepath_str in physics_nodes {  
                let full_path_string = root_nodepath.clone().to_string() + "/" + &nodepath_str;
                let nodepath = NodePath::from(&full_path_string);

                if let Some(serde_json::Value::Object(object_state)) = objects_state.get(&nodepath_str) {
                    if let Some(mut co2d) = self.base().try_get_node_as::<CollisionObject2D>(&nodepath) {
                        let this_rid = co2d.get_rid();    
                        //self.load_node(rid, data)
                        ...
                        //     let mut this_owner_shapes: Vec<ExportStateData> = Vec::new();
            ...
            for (object_nodepath, object_state) in objects_state {
                let full_object_path = format!("{}/{}", root_nodepath, object_nodepath);
        ...
        if let Some(root_node) = &self.root_node {
            ...
            for nodepath_str in physics_nodes {  
                ...
                let collated_state: CollatedObjectState = {
                    ...
                    if let Some(mut co2d) = self.base().try_get_node_as::<CollisionObject2D>(&nodepath) {
                        let this_rid = co2d.get_rid();
                        owned_states.self_state = self.get_physics_node_state(this_rid);
                ...
                SerializationFormat::None | SerializationFormat::GodotBase64 => {
                    ...
                    if let serde_json::Value::Object(map) = &mut full_physics_server_state {
                        let root_node_as_val = match serde_json::to_value(root_nodepath_string) {
                            Ok(val) => val,
                            Err(_) => {
                                godot_error!("Failed to serialize root node path!");
                                return Variant::nil()
                            },
                        };
                        ...
                        let physics_server_index_as_val = match serde_json::to_value(physics_server_index) {
                            Ok(val) => val,
                            Err(_) => {
                                godot_error!("Failed to serialize physics server index!");
                                return Variant::nil()
                            },
                        };
                        map.insert("physics_server_id".to_string(), physics_server_index_as_val);

                        let space_as_json = serde_json::to_value(space_state);

                        if let Err(err) = space_as_json {
                            godot_error!("Error serializing space state: {}", err);
                            return Variant::nil(); // Or handle the error appropriately
                        }

                        // Having handled the error case, it's safe to unwrap.
                        let space_map = space_as_json.unwrap();

                        if let serde_json::Value::Object(space_map) = space_map {
                        ...
                        for object_state in object_states {
                            ...
                            match serde_json::to_value(object_state.self_state) {
                                Ok(object_state_value) => {
                                    // Grab that value as a map.                            
                                    let object_map = match object_state_value {
                                        serde_json::Value::Object(object_map) => {
                                            // Push that map into our objects map.
                                            if let Some(objects_map) = physics_objects_state.as_object_mut() {
                                                objects_map.insert(object_state.node_path, serde_json::Value::Object(object_map));
                                            }
                                        },
                                        _ => continue,
                                    };
                                ...
                                Err(_) => {
                                    godot_print!("Unable to serialize object!");
                                    continue;
                                },
                            }                                                     
                        }
