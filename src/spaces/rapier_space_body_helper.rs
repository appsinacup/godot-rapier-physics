...
impl RapierSpace {
    pub fn is_handle_excluded_callback(
        ...
        let (collision_object_2d, _) = RapierCollisionObjectBase::get_collider_user_data(user_data, physics_rids);
                    ...
                    physics_shapes.get(&p_body.get_base().get_shape(physics_rids, body_shape_idx))
                {
                    ...
                    for result_idx in 0..result_count {
                        ...
                        let (shape_col_object, shape_index) =
                            RapierCollisionObjectBase::get_collider_user_data(&result.user_data, physics_rids);
                            ...
                            physics_collision_objects.get(&shape_col_object)
                        {
                            if let Some(collision_body) = shape_col_object.get_body() {
                                ...
                                if let Some(col_shape) = physics_shapes.get(
                                    &collision_body
                                        .get_base()
                                        .get_shape(physics_rids, shape_index),
                                ) {
    ...
    fn body_motion_collide(
        ...
        for body_shape_idx in from_shape..to_shape {
            ...
            let body_shape = p_body
                .get_base()
                .get_shape(physics_rids, body_shape_idx as usize);
