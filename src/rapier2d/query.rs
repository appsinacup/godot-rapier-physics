...
pub extern "C" fn shape_collide(pixel_motion1 : &Vector, shape_info1: ShapeInfo, pixel_motion2 : &Vector, shape_info2: ShapeInfo) -> ShapeCastResult {
    ...
    let toi_result = parry::query::cast_shapes(&shape_transform1, &shape_vel1, shared_shape1.as_ref(), &shape_transform2, &shape_vel2, shared_shape2.as_ref(), shape_cast_options);
...
pub extern "C" fn shape_casting(world_handle : Handle, pixel_motion : &Vector, shape_info: ShapeInfo, collide_with_body: bool, collide_with_area: bool, handle_excluded_callback: QueryHandleExcludedCallback, handle_excluded_info: &QueryExcludedInfo, ignore_intersecting: bool) -> ShapeCastResult {
    ...
    if let Some((collider_handle, hit)) = physics_world.query_pipeline.cast_shape(
        &physics_world.rigid_body_set, &physics_world.collider_set, &shape_transform, &shape_vel, shared_shape.as_ref(), shape_cast_options, filter
    ) {
        result.collided = true;
        result.toi = hit.time_of_impact;
        result.normal1 = Vector{ x: hit.normal1.x, y: hit.normal1.y };
        ...
        result.pixel_witness1 = vector_meters_to_pixels(&Vector{ x: witness1.x, y: witness1.y });
        result.pixel_witness2 = vector_meters_to_pixels(&Vector{ x: witness2.x, y: witness2.y });
    }
    return result;
}
